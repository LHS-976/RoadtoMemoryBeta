...
public class EnemyPatrolState : EnemyBaseState
{
    ...
    public override void OnUpdate()
    {
        ...
        if(!_enemyController.agent.pathPending && _enemyController.agent.remainingDistance <= _enemyController.agent.stoppingDistance + 0.1f)
        {
            ...
            if(waitTimer >= _enemyController.enemyManager._enemyStats.patrolidleTime)
            {
                SetRandomDestination();
                waitTimer = 0;
            }
    ...
    {
        _enemyController.agent.stoppingDistance = _enemyController.enemyManager._enemyStats.attackRange;
    }

    private void SetRandomDestination()
    {
        Vector3 randomPoint = _enemyController.spwnPosition + Random.insideUnitSphere * _enemyController.enemyManager._enemyStats.patrolRadius;

        NavMeshHit hit;
        if(NavMesh.SamplePosition(randomPoint, out hit, _enemyController.enemyManager._enemyStats.patrolRadius, NavMesh.AllAreas))
        {
    ...
    {
        Collider[] colliders = Physics.OverlapSphere(_enemyController.transform.position, _enemyController.enemyManager._enemyStats.viewRadius,
            _enemyController.enemyManager._enemyStats.playerMask);
